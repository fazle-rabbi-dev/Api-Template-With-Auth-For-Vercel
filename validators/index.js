  ...
  body("username")
    ...
    .custom((value, { req }) => {
      if (!validateUsername(value)) {
        throw new Error(
          "Username is invalid. Username must be start with a character."
        );
      }
      return true;
    }),
...
export const login = [
  body("email")
    ...
    .custom((email, { req }) => {
      ...
      if (!email && !username) {
        throw new Error(
          "Missing username or email. A username or email address is required to proceed."
        );
      }
  ...
  body("userId")
    ...
    .custom((userId, { req }) => {
      if (!validateDocumentId(userId)) {
        throw new Error(
          "Invalid user ID. Please ensure you provide a valid userId in the request body."
        );
      }
...
export const resendAccountConfirmationEmail = [
  query("email")
    ...
    .notEmpty()
    .withMessage(
      "Email address is required. Please provide a valid email address as the query parameter."
    )
    .isEmail()
  ...
  query("confirmationToken")
    ...
    .custom((confirmationToken, { req }) => {
      if (!confirmationToken) {
        throw new Error(
          "Oops! Looks like you forgot to include confirmationToken in the query parameter."
        );
      }
      return true;
    })
  ...
  query("userId")
    ...
    .custom((value, { req }) => {
      if (!validateDocumentId(value)) {
        throw new Error(
          "Invalid user id. Please ensure you provide a valid userId in the query parameter."
        );
      }
      return true;
    }),
...
export const changeEmail = [
  body("newEmail")
    ...
    .isEmail()
    .withMessage(
      "Invalid new email address. Please ensure you provide a valid new email address."
    ),
  body("password")
    .trim()
    .isLength({ min: 6 })
